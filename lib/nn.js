...
class NeuralNetwork {
  constructor(input_nodes, hidden_nodes, output_nodes) {
    ...
    let ihVariance = sqrt((input_nodes + hidden_nodes) * 0.5);
    let hoVariance = sqrt((output_nodes + hidden_nodes) * 0.5);

    this.weights_ih = dl.variable(dl.randomNormal([this.hidden_nodes, this.input_nodes], 0, 1 /
      ihVariance));
    this.weights_ho = dl.variable(dl.randomNormal([this.output_nodes, this.hidden_nodes], 0, 1 /
      hoVariance));
    this.bias_h = dl.variable(dl.randomNormal([this.hidden_nodes, 1]));
    this.bias_o = dl.variable(dl.randomNormal([this.output_nodes, 1]));
  ...
  predictBatch(input_arrays) {
    return dl.tidy(() => {
      let ia = dl.cast(dl.tensor2d(input_arrays), 'float32');
      let il = input_arrays[0].length;
      let results = [];
      for (let i = 0, end = input_arrays.length; i < end; ++i) {

        // let inputs = dl.tensor2d(input_arrays[i], [input_arrays[i].length, 1]);
        let inputs = ia.slice([i, 0], [1, il])
          .reshape([il, 1]); //dl.tensor2d(input_arrays[i], [input_arrays[i].length, 1]);
        ...
        // Sending back to the caller!
        results.push(outputs.getValues());
      }
      return results;
    });
  }
    ...
    dl.tidy(() => {
      ...
      for (let i = 0, end = input_arrays.length; i < end; ++i) {
        dl.tidy(() => {
          ...
          // Convert array to matrix object
          // let targets = Matrix.fromArray(target_array);
          let targets = dl.tensor2d(target_arrays[i], [target_arrays[i].length, 1],
            'float32');
