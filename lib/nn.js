...
class NeuralNetwork {
  constructor(input_nodes, hidden_nodes, output_nodes) {
    ...
    dl.tidy(() => {
      ...
      this.hidden_activations = dl.variable(
        dl.zeros([this.hidden_nodes, 1]),
        true,
        'activation1'
      );
      this.output_activations = dl.variable(
        dl.zeros([this.output_nodes, 1]),
        true,
        'activation2'
      );
    });
  ...
  predict(input_array) {
    ...
    return dl.tidy(() => {
      ...
      let inputs;
      if (input_array instanceof dl.Tensor) inputs = input_array;
      else inputs = dl.tensor2d(input_array, [input_array.length, 1]);

      this.hidden_activations.assign(dl.matMul(this.weights_ih, inputs));

      this.hidden_activations.assign(
        dl.add(this.hidden_activations, this.bias_h)
      );
      ...
      this.output_activations.assign(
        dl.matMul(this.weights_ho, this.hidden_activations)
      );

      this.output_activations.assign(
        dl.add(this.output_activations, this.bias_o)
      );
  ...
  setLearningRate(learning_rate = 0.1) {
    if (this.learning_rate instanceof dl.Tensor) this.learning_rate.dispose();
  ...
  train(input_array, target_array) {
    ...
    dl.tidy(() => {
      ...
      let gradients = dl.mul(
        this.output_activations,
        dl.sub(dl.scalar(1), this.output_activations)
      );
      ...
      let hidden_gradient = dl.mul(
        this.hidden_activations,
        dl.sub(dl.scalar(1), this.hidden_activations)
      );
      hidden_gradient = dl
  ...
  predictBatch(input_arrays) {
    ...
    return dl.tidy(() => {
      let inputs;
      if (input_arrays instanceof dl.Tensor) inputs = input_arrays;
      else inputs = dl.tensor2d(input_arrays);
  ...
  trainBatch(input_arrays, target_arrays) {
    ...
    dl.tidy(() => {
      ...
      // Back propagation
      let targets;
      if (target_arrays instanceof dl.Tensor) targets = target_arrays;
      else targets = dl.tensor2d(target_arrays);

      // Calculate the error
      // ERROR = TARGETS - OUTPUTS
      let output_errors = dl.sub(targets, outputs).mul(this.learning_rate);
      ...
      this.bias_o.assign(
        dl.add(
          this.bias_o,
          dl.matMul(gradients.transpose(), dl.ones([gradients.shape[0], 1]))
        )
      );
      this.bias_h.assign(
        dl.add(
          ...
          dl.matMul(
            ...
            dl.ones([hidden_gradient.shape[0], 1])
          )
        )
      );
    });
